      ...
      <section className="border rounded p-3 space-y-3">
        ...
        {invite && (
          <div className="space-y-1">
            <p><b>Code:</b> {invite.code}</p>
            <p>
              <b>Link:</b>{" "}
              <a className="underline" href={invite.link}>{invite.link}</a>
            </p>
          </div>
        )}
      </section>
            ...
            {friends.map((f) => (
              <div key={f.friendId} className="border rounded p-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm opacity-80">id: {f.friendId} • {f.status}</p>
                </div>
              ...
              <div key={`${item.friendId}-${item.at}-${idx}`} className="border rounded p-3">
                ...
                <div className="mt-2 text-sm space-y-1">
                  {(item.level != null || item.xp != null) && (
                    <p><b>Level:</b> {item.level ?? "—"} &nbsp; <b>XP:</b> {item.xp ?? "—"}</p>
                  )}
                  {item.streak != null && <p><b>Streak:</b> {item.streak}</p>}
                  {item.category && <p><b>Category:</b> {item.category}</p>}
                  {item.momentText && <p><b>Moment:</b> {item.momentText}</p>}
                </div>
              </div>
            ))}
          </div>
